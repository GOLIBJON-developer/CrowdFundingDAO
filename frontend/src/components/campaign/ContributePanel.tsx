...
export function ContributePanel({
  ...
  const { contribute, hash, isPending, isConfirming, isSuccess, error, reset } =
  ...
  if (isSuccess) {
    return (
      <div className="p-4 rounded-lg border border-emerald-500/25 bg-emerald-500/5">
        <p className="text-emerald-400 font-mono text-sm font-medium mb-1">
          ✓ Contribution confirmed
        </p>
        <p className="text-zinc-500 font-mono text-xs">
          You received FUND governance tokens.
        </p>
        <button
          onClick={() => { reset(); setAmount(''); onSuccess?.() }}
  ...
  return (
    <div className="p-4 rounded-lg border border-zinc-800 bg-zinc-900/40">
      <div className="flex items-center justify-between mb-4">
        ...
        {address && (myContribution as bigint) > 0n && (
          <span className="text-xs font-mono text-zinc-500">
            Your total:{' '}
            <span className="text-amber-400">{fmtEth(myContribution as bigint)} ETH</span>
          </span>
        )}
      </div>

      {disabledReason ? (
        <p className={`text-xs font-mono py-2 ${isPaused ? 'text-orange-400' : 'text-zinc-600'}`}>
          {disabledReason}
        </p>
      ) : !address ? (
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div className="relative">
            ...
            />
            <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs font-mono text-zinc-500">
              ETH
            </span>
          </div>
